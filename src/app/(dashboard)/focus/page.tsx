  ...
  return (
    <div className="space-y-10">
      ...
      <section className="panel space-y-4">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <h2 className="text-sm font-semibold text-[#0b0d0f]">Commit a daily focus</h2>
          {dailyFocus?.project && <span className="rounded-full border border-[rgba(0,0,0,0.08)] bg-[#f8f9fa] px-3 py-1 text-xs text-[#555]">Committed: {dailyFocus.project.name}</span>}
        </div>
        <form action={setDailyProject} className="flex flex-wrap items-center gap-2 text-sm">
          <select name="projectId" defaultValue={dailyFocus?.projectId ?? ""} className="border border-[rgba(0,0,0,0.12)] bg-white px-3 py-2">
            <option value="">Choose a project</option>
            {activeProjects.map((p) => (
              <option key={p.id} value={p.id}>{p.name}</option>
            ))}
          </select>
          <button type="submit" className="rounded-md border border-[rgba(0,0,0,0.12)] px-4 py-2 text-sm font-semibold">Commit</button>
        </form>
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="panel space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-[#0b0d0f]">Pinned for today</h2>
            <span className="text-xs text-[#555]">Up to 3 tasks</span>
          </div>
          <div className="space-y-2">
            {pinnedItems.map((item) => (
              <div key={item.id} className="flex items-center justify-between rounded border border-[rgba(0,0,0,0.06)] bg-[#f8f9fa] px-3 py-2 text-sm">
                <div>
                  <div className="font-semibold text-[#0b0d0f]">{item.title}</div>
                  {item.projectId && <div className="text-xs text-[#555]">Project task</div>}
                </div>
        ...
        <div className="panel space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-[#0b0d0f]">Suggested tasks</h2>
            <span className="text-xs text-[#555]">From active projects</span>
          </div>
          <div className="space-y-2">
            {unpinnedSuggestions.map((item) => (
              <div key={item.id} className="flex items-center justify-between rounded border border-[rgba(0,0,0,0.06)] bg-[#f8f9fa] px-3 py-2 text-sm">
                <div>
                  <div className="font-semibold text-[#0b0d0f]">{item.title}</div>
                  <div className="text-xs text-[#555]">{item.project?.name ?? "Project"}</div>
                </div>
                {!pinnedIds.has(item.id) && pinnedItems.length < 3 && (
                  <form action={pinItem}>
                    <input type="hidden" name="itemId" value={item.id} />
                    <button className="rounded-md border border-[rgba(0,0,0,0.12)] px-3 py-1 text-xs font-semibold">Pin</button>
                  </form>
                )}
              </div>
            ))}
      ...
      <section className="panel space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold text-[#0b0d0f]">Time boxes</h2>
          <span className="text-xs text-[#555]">Total: {totalMinutes} min</span>
        </div>
        <form action={addSession} className="flex flex-wrap gap-2 text-sm">
          <input name="label" placeholder="Focus block" className="border border-[rgba(0,0,0,0.12)] bg-white px-3 py-2 flex-1" required />
          <input name="minutes" type="number" min={15} step={5} placeholder="Minutes" className="border border-[rgba(0,0,0,0.12)] bg-white px-3 py-2 w-28" required />
          <button type="submit" className="rounded-md border border-[rgba(0,0,0,0.12)] px-4 py-2 text-sm font-semibold">Add</button>
        </form>
        <div className="space-y-2">
          {sessions.map((s) => (
            <div key={s.id} className="flex items-center justify-between rounded border border-[rgba(0,0,0,0.06)] bg-[#f8f9fa] px-3 py-2 text-sm">
              <div className="font-semibold text-[#0b0d0f]">{s.label}</div>
