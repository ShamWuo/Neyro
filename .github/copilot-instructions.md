# Copilot instructions — checklist

Work through each checklist item systematically. Keep communication concise and focused. Follow development best practices.

## Checklist

- [x] Verify that the `copilot-instructions.md` file in the `.github` directory is created.
- [x] Clarify Project Requirements
- [x] Scaffold the Project
- [x] Customize the Project
- [x] Install Required Extensions (none needed)
- [x] Compile the Project
- [x] Create and Run Task (not needed)
- [ ] Launch the Project (pending deployment/env review)
- [x] Ensure Documentation is Complete

## Notes

- Converted to Markdown checklist and removed stray HTML comment markers.
- `Launch the Project` remains unchecked because deployment/environment review is pending.
