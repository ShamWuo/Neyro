...
import { Mic, Image as ImageIcon, Link2, Clipboard, ChevronDown, Check, X } from 'lucide-react';
...
function classifyInput(text: string): { category: PARACategory; reason: string; label: string; icon: string; sub: string } {
    ...
    if (/finish|complete|build|launch|create|deadline|by (monday|tuesday|wednesday|thursday|friday|saturday|sunday|jan|feb|mar|next week)|due|ship|deliver/.test(lower)) {
        return { category: 'projects', label: 'PROJECT', icon: '📁', sub: 'Things with a finish line', reason: 'I detected a goal with a clear outcome — this has a finish line.' };
    }
    if (/health|fitness|workout|exercise|finance|budget|money|savings|relationship|family|friend|career|work|job|learning|habit|routine|weekly|monthly/.test(lower)) {
        return { category: 'areas', label: 'AREA', icon: '🔄', sub: 'Ongoing responsibilities', reason: 'This sounds like an ongoing responsibility, not a one-time task.' };
    }
    if (/article|read|research|idea|note|reference|link|book|learn|interesting|thoughts on|how to|guide|template/.test(lower)) {
        return { category: 'resources', label: 'RESOURCE', icon: '📚', sub: 'Reference material', reason: 'This looks like reference material you\'ll want to come back to.' };
    }

    return { category: 'projects', label: 'PROJECT', icon: '📁', sub: 'Things with a finish line', reason: 'Most captured items end up being actionable projects.' };
}
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                if (classification.category === 'projects') {
                    setTimeout(() => {
                        toast({
                            title: `💡 This connects to 'Investor Pitch Deck' from 3 days ago.`,
                            variant: 'default',
                        ...
                        >
                            ...
                            <div className="px-4 pb-3">
                                ...
                                <div className="flex items-center gap-2 mt-2">
                                    <button className="px-2.5 py-1.5 border border-border rounded-button text-text-muted hover:text-primary hover:bg-hover transition-colors flex items-center gap-1.5 text-[11px] font-semibold tracking-[0.02em]">
                                        <Mic size={12} /> Voice
                                    </button>
                                    <button className="px-2.5 py-1.5 border border-border rounded-button text-text-muted hover:text-primary hover:bg-hover transition-colors flex items-center gap-1.5 text-[11px] font-semibold tracking-[0.02em]">
                                        ...
                                        <div className="bg-card border border-border rounded-card p-3 shadow-sm flex items-start gap-3">
                                            ...
                                            <div className="flex-1">
                                                <div className="flex items-center justify-between mb-1">
                                                    ...
                                                    <button className="text-[11px] text-text-muted hover:text-primary transition-colors flex items-center gap-1">
