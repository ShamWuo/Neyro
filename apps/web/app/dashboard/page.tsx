...
export default function DashboardPage() {
    ...
    // Better way to handle hydration/mounting for React 18+ to avoid lint error
    ...
    // Memoize the activityFeed to stay pure during render
    const activityFeed = useMemo(() => {
        return captures.slice(0, 5).map((capture) => {
            const diffMs = Math.abs(capture.timestamp.getTime() - renderNow);
            const diffDays = Math.ceil(diffMs / (1000 * 60 * 60 * 24));
            const timeAgo = diffDays > 0 ? `${diffDays}d ago` : 'Today';
            const categoryBase = capture.category.toLowerCase().split(' ')[0].replace('(', '');
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 mt-8">
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-1">
                        {activityFeed.map((capture) => (
                                <div 
                                    key={capture.id} 
                                    onClick={() => router.push(`/para/${capture.categorySlug}`)}
                                    className="flex gap-4 group py-3 px-3 -mx-3 hover:bg-hover rounded-card transition-colors cursor-pointer border border-transparent hover:border-border/50"
                                >
                                    ...
                                    <div className="flex-1 space-y-1.5">
                                        ...
                                        <div className="flex justify-between items-center">
                                            ...
                                            <span className="text-[11px] font-mono text-text-placeholder">{capture.timeAgo}</span>
                                        </div>
                                    </div>
                                </div>
                            ))}
