...
import { Archive, RotateCcw, Trash2, Search, Filter, CheckCircle2 } from 'lucide-react';
...
export default function ArchivePage() {
    ...
    const updateResource = useNeyroStore((state) => state.updateResource);
    ...
    const archivedProjects = useMemo(() => {
        const allProjects = [...(completedProjects || []), ...(pausedProjects || [])];
    ...
    const handleRestoreProject = useCallback(async (id: string) => {
        const now = Date.now();
        await updateProject(id, { status: 'active', completedAt: null, updatedAt: now });
    }, [updateProject]);

    const handleRestoreResource = useCallback(async (id: string) => {
        const now = Date.now();
        await updateResource(id, { isArchived: false, updatedAt: now });
    }, [updateResource]);
