...
import { Sparkles, CheckCircle2, Circle, Clock, ArrowRight, BatteryCharging } from 'lucide-react';
            ...
            <div className="lg:w-[320px] shrink-0 border-t lg:border-t-0 lg:border-l border-border pt-8 lg:pt-0 lg:pl-8 space-y-10">
                ...
                >
                    ...
                    <div className="p-4 bg-accent-light/30 border-l-2 border-l-accent rounded-r-card text-sm space-y-3">
                        ...
                        <button 
                            onClick={() => {
                                const { addProjectTask } = useDemoStore.getState();
                                addProjectTask('1', 'Draft slide: Key Traction Metrics');
                                addProjectTask('1', 'Draft slide: Growth Projections');
                                addProjectTask('1', 'Draft slide: User Retention Analysis');
                                toast.success("Neyro AI drafted 3 slides in &apos;Investor Pitch Deck&apos;");
                            }}
                    ...
                    <div className="grid grid-cols-5 gap-1.5">
                        {habits.slice(0, 5).map((habit, i) => (
                            <div key={habit.id} className="group relative">
                                <div 
                                    onClick={() => {
                                        const { addHabitEntry } = useDemoStore.getState();
                                        addHabitEntry(habit.id);
                                        toast.success(`Toggled habit: ${habit.name}`);
                                    }}
                                    className={cn(
                                        "w-full aspect-square rounded-[4px] border transition-colors flex items-center justify-center cursor-pointer",
                                        i < 3 || habit.streak > 0 // Show first 3 or active streak as completed for mock purposes
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                        {recentCaptures.map(capture => (
                            <div 
                                key={capture.id} 
                                onClick={() => router.push(`/para/resources?search=${encodeURIComponent(capture.content)}`)}
                                className="p-3 bg-subtle rounded-card border border-transparent hover:border-border transition-colors text-sm cursor-pointer group"
                            >
