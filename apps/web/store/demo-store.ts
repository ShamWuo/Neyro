...
export const useDemoStore = create<DemoState>((set) => ({
    ...
    addHabitEntry: (habitId) => set((state) => ({
        habits: state.habits.map(h => 
            h.id === habitId 
                ? { ...h, completedDays: h.completedDays + 1, streak: h.streak + 1 } 
                : h
        ),
        momentumScore: Math.min(100, state.momentumScore + 5)
    })),
