...
async function testStripeInitialization() {
  ...
  try {
    ...
    log(`❌ Stripe API key validation failed: ${error.message}`, "error");
    return false;
  }
}
    ...
    if (!priceId || isFallback) {
      ...
      if (stripe) {
        try {
          ...
          log(`   ⚠️  Price not found in Stripe: ${error.message}`, "warn");
        }
      }
    }
  }
...
async function testCheckoutSessionCreation() {
  ...
  try {
    ...
    log(`❌ Checkout session creation failed: ${error.message}`, "error");
    return false;
  }
}
...
async function testPortalSessionCreation() {
  ...
  try {
    ...
    log(`❌ Portal session creation failed: ${error.message}`, "error");
    return false;
  }
}
...
async function testWebhookSecret() {
  ...
  if (stripe) {
    try {
      ...
      log(`⚠️  Webhook verification test: ${error.message}`, "warn");
    }
  }
